...
int main(int argc,char *argv[]){
    ...
    stop = chrono::high_resolution_clock::now();
    duration = chrono::duration_cast<chrono::microseconds>(stop - start);
    cout << "Adding bytes from root to leaf takes: " << duration.count() << " microseconds" << endl;
...
void write_the_file_content(FILE *original_fp,long int size,string *str_arr,unsigned char &current_byte,int &current_bit_count,FILE *compressed_fp){
    ...
    cout << "Image Encoding takes: " << duration.count() << " microseconds" << endl;

    
    start = chrono::high_resolution_clock::now();
    fwrite(compressed_bytes.data(), sizeof(unsigned char), compressed_bytes.size(), compressed_fp);
