...
void write_the_file_content(char *, char*, long int,string *,unsigned char &,int &, long int [][256], long int);
void write_the_file_content_seq(FILE*,long int,string*,unsigned char&,int&,FILE*);
...
// Function to calculate entropy of the file
double calculate_entropy(long int number[], int size) {
    double entropy = 0.0;
    long int total = 0;
    
    // calculate the total count of all elements in the array
    ...
    for (int i = 0; i < size; i++) {
        double p = static_cast<double>(number[i]) / total;
        if (p != 0) {
...
int main(int argc,char *argv[]){
    ...
    if (rank == 0) {
        printf("number of process: %d", num_procs);
    }
    ...
    int NPROCESS = std::stoi(argv[--argc]);
    int NTHREAD = std::stoi(argv[--argc]);
    if (rank == 0)
        printf("\nnumber of thread: %d\n", NTHREAD);
    ...
    long int freq_thread[NTHREAD][256];
    ...
    for(int current_file=1;current_file<argc;current_file++){
        ...
        {  
            ...
            int chunk_size = size / NTHREAD;
            int start = tid * chunk_size;
            int end = (tid == NTHREAD - 1) ? size : start + chunk_size;
            long int *number_private = freq_thread[tid];

            // Seek to the start position
            fseek(fp, start, SEEK_SET);
            for (int i = start; i < end; i++) {
                fread(&buffer, 1, 1, fp);
                number_private[buffer]++;
            }
            // Reduce on number array
            #pragma omp critical
            {
                for(int i=0; i<256; ++i) {
                    number[i] += number_private[i];
                }
            }
            fclose(fp);
        }      
    ...
    //print out entropy
    double entropy = calculate_entropy(number, 256);
    entropy = (entropy - 7.72) * (8.0-1.0) + 1.0; // 7.72 is the entropy of black and white image, for normalization
    double comp_ratio = 8.0 / entropy; 
    if (comp_ratio < 1.5) {
        std::cout << "Compression ratio is too low, aborting" << std::endl;
        return 0;
    }

    auto stop = chrono::high_resolution_clock::now();
    auto duration = chrono::duration_cast<chrono::microseconds>(stop - start0);
    if (rank == 0) {
        cout << "Entropy: " << entropy << std::endl;
        cout << "Frequency analysis takes: " << duration.count() << " microseconds" << endl;
    }
    ...
    if (rank == 0) {
        cout << "Creating translation array takes: " << duration.count() << " microseconds" << endl;
    }
    //---------------------------------------------
    ...
    duration = chrono::duration_cast<chrono::microseconds>(stop - start);
    if (rank == 0) {
        cout << "Creating huffman tree takes: " << duration.count() << " microseconds" << endl;
    }
    ...
    duration = chrono::duration_cast<chrono::microseconds>(stop - start);
    if (rank == 0) {
        cout << "Adding bytes from root to leaf takes: " << duration.count() << " microseconds";
    }
    ...
    for(int current_file=1;current_file<argc;current_file++){
        ...
        // Sequential
        // write_the_file_content_seq(original_fp,size,str_arr,current_byte,current_bit_count,compressed_fp);
        // Parallel
        fclose(compressed_fp);
        char * compress_file_name = &scompressed[0];
        long int meta_size = size_of_the_file(&scompressed[0]); // Get the current position of the file pointer, which is the file size
        fclose(original_fp);
        write_the_file_content(argv[current_file],compress_file_name,size,str_arr,current_byte,current_bit_count, freq_thread, meta_size);      //writes eighth
        compressed_fp=fopen(compress_file_name,"ab");
    }
    ...
    stop = chrono::high_resolution_clock::now();
    duration = chrono::duration_cast<chrono::microseconds>(stop - start);
    auto total = chrono::duration_cast<chrono::microseconds>(stop - start0);

    if (rank == 0) {
        cout<<endl<<"Created compressed file: "<<scompressed<<endl;
        cout<<"Compression is complete"<<endl;
        cout << "Writing compressed file takes: " << duration.count() << " microseconds" << endl;
        cout << "IN TOTAL: " << total.count() << " microseconds" << endl; 
    }
...
void write_the_file_content(char *original_file_name, char *compress_file_name, long int size,string *encoding_tb,unsigned char &current_byte,int &current_bit_count, long int freq_thread[][256], long int meta_size){
    unsigned char *x_p,x;
    x_p=&x;
    char *encode_value;
    ...
    for (int i = 0; i < 256; i++) {
        encode_length[i] = encoding_tb[i].size();
    }

    // Calculate the size of subsections for each thread
    int NTHREAD = omp_get_num_threads();
    int thread_end[NTHREAD] = {0};
    for (int tid = 0; tid < NTHREAD; tid++) {
        auto freq = freq_thread[tid];
        for (int i = 0; i < 256; i++) {
            // printf("%ld ", freq[i]);
            long int count = freq[i];
            thread_end[tid] += encode_length[i] * count;
        }
    }
    // create a NTHREAD number of vector
    int NUMCHUNK = 100;
    int chunk_size = size / NUMCHUNK;
    vector<unsigned char> compressed[NUMCHUNK];

    auto start = chrono::high_resolution_clock::now();
    #pragma omp parallel num_threads(NTHREAD)
    {   
        FILE *og_fp = fopen(original_file_name,"rb");
        #pragma omp for schedule(dynamic)
        for(long int i=0; i<NUMCHUNK; i++){
            int start = i * chunk_size;
            int end = (i == NUMCHUNK - 1) ? size : start + chunk_size;
            char *encode_value;
            unsigned char *x_p,x;
            x_p=&x;

            // Seek to the start position for both orignal file and compressed file
            fseek(og_fp, start, SEEK_SET); // orignal file

            // encoding process
            fread(x_p,1,1,og_fp);
            for(long int i=start; i<end; i++){
                encode_value=&encoding_tb[x][0];
                while(*encode_value){
                    if(current_bit_count==8){
                        compressed[i].push_back(current_byte);
                        current_bit_count=0;
                    }
                    switch(*encode_value){
                        case '1':current_byte<<=1;current_byte|=1;current_bit_count++;break;
    ...
    cout << endl << "Image Encoding takes: " << duration.count() << " microseconds" << endl;

    start = chrono::high_resolution_clock::now();
    FILE *cp_fp = fopen(compress_file_name, "ab");
    // for each thread, write the compressed vector to file
    for (int i = 0; i < NUMCHUNK; i++) {
        fwrite(&compressed[i][0], 1, compressed[i].size(), cp_fp);
    }
    stop = chrono::high_resolution_clock::now();
    duration = chrono::duration_cast<chrono::microseconds>(stop - start);
    cout << "Writing compressed image takes: " << duration.count() << " microseconds" << endl;



    // vector<unsigned char> compressed_bytes;
    ...
    //             case '1':current_byte<<=1;current_byte|=1;current_bit_count++;break;
    //             case '0':current_byte<<=1;current_bit_count++;break;
    //             default:printf("%c\n", *encode_value);cout<<"An error has occurred"<<endl<<"Process has been aborted";
    //             exit(2);
    //         }
    //         encode_value++;
    //     }
    ...
    // // Compute the chunk size for each 
    // size_t dataSize = compressed_bytes.size();
    // size_t chunkSize = dataSize / num_procs;
    // size_t remainder = dataSize % num_procs;
    ...
    // size_t start = (rank * (dataSize / num_procs)) + std::min((size_t)rank, remainder);

    // // Write the raw bytes to a file using MPI-IO
    // MPI_File file;
    // MPI_File_open(MPI_COMM_WORLD, compress_file_name, MPI_MODE_WRONLY | MPI_MODE_APPEND, MPI_INFO_NULL, &file);

    // MPI_Offset offset = start * sizeof(unsigned char) + meta_size;
    // MPI_File_write_at(file, offset, &compressed_bytes[start], chunkSize, MPI_UNSIGNED_CHAR, MPI_STATUS_IGNORE);

    // MPI_File_close(&file);
    // MPI_Finalize();
}

void write_the_file_content_seq(FILE *original_fp,long int size,string *str_arr,unsigned char &current_byte,int &current_bit_count,FILE *compressed_fp){
    unsigned char *x_p,x;
    x_p=&x;
    char *str_pointer;
